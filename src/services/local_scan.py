        ...
        for scan_path in self.scan_path:
            ...
            for root, dirs, files in os.walk(scan_path):
                if not self.is_running:
                    break
                for name in dirs:
                    dir_path = os.path.normcase(os.path.normpath(os.path.join(root, name)))
                    parent_id = '' if root == scan_path else os.path.normcase(os.path.normpath(os.path.dirname(
                        dir_path)))
                    try:
                ...
                for name in files:
                    ...
                    if name.lower() == 'desktop.ini':
                        continue
                    file_path = os.path.normcase(os.path.normpath(os.path.join(root, name)))
                    parent_id = '' if root == scan_path else os.path.normcase(os.path.normpath(os.path.dirname(
                        file_path)))
